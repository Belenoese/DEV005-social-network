...
function mistake(navigateTo2) {
  ...
  <h2 class="text"> Error 404: p\xE1gina no encontrada.</h2>
...
const db = wh(app);
const ref = () => dl(sh(db, "posts"));
const colPost = sh(db, "posts");
const post = (postText) => {
  uf(colPost, {
    text: postText,
...
const editPost = (id2, text) => {
  const postRef = rh(db, "posts", `${id2}`);
  rf(postRef, {
    text
  });
};
const deleteDocData = (id2) => {
  of(rh(db, "posts", `${id2}`));
};
const like = (id2, email) => {
  const postRef = rh(db, "posts", `${id2}`);
  rf(postRef, {
    likes: Sf(email)
  });
};
const disLike = (id2, email) => {
  const postRef = rh(db, "posts", `${id2}`);
  rf(postRef, {
    likes: Df(email)
  });
...
function home(navigateTo2) {
  ...
  const printPost = (info, doc) => {
    ...
    postContainer.appendChild(name2);
    name2.innerHTML = `${info.userEmail}`;
    ...
    deleteButton.addEventListener("click", () => {
      const confirmDelete = confirm("\xBFEst\xE1s seguro que deseas eliminar este post?");
      if (confirmDelete) {
...
function init(navigateTo2) {
  ...
  section.innerHTML = `
    ...
    <button class='logIn'>Inicia Sesi\xF3n</button>
    <button class='register'>Reg\xEDstrate</button>
    ...
    <button class='google'>Reg\xEDstrate con Google</button>
  `;
...
function login(navigateTo2) {
  ...
  <img src='https://firebasestorage.googleapis.com/v0/b/social-network-dev-005.appspot.com/o/assets%2FLogo.png?alt=media&token=feddabaf-9c84-4263-a12f-7244c595f987' class= 'logo2'>
    ...
    <button class="bottomTextLogin">\xBFNo tienes una cuenta? Reg\xEDstrate</button>
    </section >
    `;
...
function create(navigateTo2) {
  ...
  sectionCreate.innerHTML = `
    ...
    <label class="textPasswordCreateAccount">Contrase\xF1a nueva</label>
    <input class="card2" id="card2" type="password" placeholder='********'></input>
    <span class="alerta"></span>
    <button class="bottomKeep" id="btnguardar">Guardar</button>
    </form>
    <button class="bottomText">\xBFYa tienes cuenta? Iniciar Sesi\xF3n</button>
    </section>
    `;
